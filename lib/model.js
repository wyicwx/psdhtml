    ...
    parse () {
        ...
        if (this.type === 'Group') {
            ...
            if (isFlow) {
                ...
                // 如果父节点flow, 本身没有flow_left也对齐
                if (parentIsFlow && !groupRules || (groupRules && groupRules.indexOf('flow_left') === -1)) {
                    this.dom.style['margin-right'] = 'auto';
                    this.dom.style['margin-left'] = 'auto';
                }
            }
            ...
            if (this.node.hasChildren()) {
                ...
                this.node.children().reverse().forEach((node, index) => {
                    ...
                    if (grule && grule.indexOf('bg') !== -1) {
                        ...
                        // 如果是在flow模式下，则100%
                        if (isFlow) {
                            this.dom.style['background-size'] = '100% 100%';
                        }
                ...
                // 如果父节点是flow, 同时本身只有一个text节点，则不取高度
                if (parentIsFlow && this.childrens && this.childrens.length === 1 && this.typeChildrens['Text'].length === 1) {
                    this.dom.style.height = null;
                    // 同时，改text节点去掉绝对定位
                    let node = this.typeChildrens['Text'][0];
                    node.dom.style.position = null;
                    node.dom.style.left = null;
                    node.dom.style.right = null;
                }
                // 如果是flow,计算padding-bottom
                // 找到最后一个非absolute的元素(bottom 最大的)
                if (isFlow) {
                    let nodes = this.typeChildrens['Group'];
                    let lastNoAbsoluteGroup;
                    for (let i = 0; i < nodes.length; i++) {
                        let node = nodes[i];
                        if (i === 0) {
                            lastNoAbsoluteGroup = node;
                        }
                        if (!node.isAbsolute) {
                            if (node.layer.bottom > lastNoAbsoluteGroup.layer.bottom) {
                                lastNoAbsoluteGroup = node;
                            }              
                        }
                    }
                    if (lastNoAbsoluteGroup) {
                        this.dom.style['padding-bottom'] = this.layer.bottom - lastNoAbsoluteGroup.layer.bottom;
                    }
                }
            }
            ...
            if (this.type === 'Text') {
                ...
                this.dom.style['line-height'] = '$line_height';
