    ...
    parse () {
        ...
        if (this.type === 'Group') {
            ...
            if (this.typeIndex === 0) {
                ...
                offsetTop = this.layer.top - this.parent.typeChildrens[this.type][this.typeIndex - 1].layer.bottom;
            }
            ...
            if (this.node.hasChildren()) {
                this.node.children().reverse().forEach((node, index) => {
                    ...
                    if (model.rule.group && model.rule.group.indexOf('bg') !== -1) {
                        this.dom.style.background = `url(../images/${model.name}.png) no-repeat;`;
